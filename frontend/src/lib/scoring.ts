...
/**
 * Deterministic compliance scoring. The score is a penalty model: every
 * environment starts at 100 and each open drift finding subtracts a fixed
 * weight by severity. The result is clamped to [0, 100]. No randomness, no LLM —
...
export function computeComplianceScore(summary: SeverityCounts): number {
  const penalty = SEVERITY_ORDER.reduce(
    (sum, s) => sum + SEVERITY_WEIGHTS[s] * summary[s],
    0,
  );
...
export function scoreBreakdown(summary: SeverityCounts) {
  return SEVERITY_ORDER.map((severity) => ({
    ...
    penalty: SEVERITY_WEIGHTS[severity] * summary[severity],
  }));
}
